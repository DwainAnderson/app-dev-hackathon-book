...
class KNNModel(object):
    ...
    def generate_n_dimensional_space(self):
        ...
        ax.scatter(n_space[:,0], n_space[:,1], n_space[:,2])  # Example for 3 dimensions
        ax.set_xlabel('Rating')
        ax.set_ylabel('Genre1')
        ax.set_zlabel('Genre2')
        plt.show()

    def recommendation_algorithm(self, user_id):
        ...
        favorite_books = self.db.get_user_favorites(user_id)
        ...
        for i, book in enumerate(self.books):
            _, _, _, _, _ = book
            book_genres = self.db.get_book_genres(book_id)
            book_rating = self.db.get_rating(book_id)
            ...
            recommended_books.append((book_id, distance))
